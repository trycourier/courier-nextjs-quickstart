...
// Next reads .env.local automatically; a standalone script does not.
...
async function main() {
  ...
  if (!apiKey) {
    console.error("COURIER_API_KEY is not set. Copy .env.example to .env.local and add your key.");
    process.exit(1);
  }
  ...
  console.log("It should already be in the inbox at http://localhost:3000");
}
